...
N_rods = 74; %Number of fuel rods
N_assemblies = 764;
Q_assemblies = Q_core / N_assemblies;
Q_rod = Q_assemblies/N_rods;
rho_Zr = 6511; %Kg/m3; %Density of the cladding 
%V_clad = (pi*((clad_diam_outer/2).^2)-pi*((clad_diam_inner/2).^2))*(clad_length); %Volume of the cladding in m3
...
P_in = 1; %bar
T_in = 100;% °C
mu = 0.0002814; % Viscosity of water in Pascal*s
rho_in = XSteam('rhoL_p',P_in); % Density of water Kg/ m3
g = 9.81; % Accelleration gravity m/s2
%k_w = XSteam('tcV_p',P_in) ; % Therman conductivity in W/m°C or W/mK of water
k_w = 0.68;
...
Q_rod = Q_rod*0.04; %Watt power of the fuel rod;
Pr = (cp*mu/k_w);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Time necessary & Temp to achieve onset of nucleate boling
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
int = linspace(min(T_wall_nc),max(T_wall_nc),10000);
...
if rc > rc_max    
          ...
          T_onb = Temp_onb(end)
          T_onb_celsius = Temp_onb(end)-273.15
          q_ONB=polyval(cnc,T_onb)
...
heat_difference(ii) = Q_rod - qs_nb_R(ii).*Wetted_area;
end
...
figure(6)
plot(time_nb_R, heat_difference,'LineWidth',2)
...
t_stop_1 = time_nb_R(ii)

for kk=1:1:ii
    time_Bi(kk)=time_nb_R(kk);
    T_wall_Bi(kk) = T_wall_nb_R(ii);

end
figure(6)
plot(time_Bi, heat_Bi,LineWidth=2)
xlabel('Time [s]')
ylabel('Heat flux variation [W/m2]')
title('Time evaluation for when Bi < 0.1')
figure(7)
plot(time_Bi, T_wall_Bi-273.15, LineWidth=2)
xlabel('Time [s]')
ylabel('T-wall-NB [°C]')
